...
from ui.utils import load_font, draw_rect_alpha, draw_rounded_rect_alpha
...
def show_card(screen: pygame.Surface,
    ...
    while True:
        ...
        if draw_background is not None:
            draw_background()
        draw_rect_alpha(screen, (0, 0, 0), (0, 0, SCREEN_W, SCREEN_H), 95)

        # 卡片背景
        draw_rounded_rect_alpha(screen, DARK_GRAY, (cx, cy, cw, ch), 188, border_radius=12)
        pygame.draw.rect(screen, style["color"], (cx, cy, cw, ch), 3, border_radius=12)

        # 卡片標題列
        draw_rounded_rect_alpha(screen, style["color"], (cx, cy, cw, 46), 205, border_radius=12)
